...
Node.prototype.setGeo = function(ip)
{
	...
	console.success('setGeo', ip, this.geo)
}
...
Node.prototype.getInfo = function(callback)
{
	return {
		...
		stats: {
			...
			nextBlockTime: this.stats.nextBlockTime
		},
...
Node.prototype.setBasicStats = function(stats, callback)
{
	if( !_.isUndefined(stats) )
	{
		if( !_.isEqual(stats, {
			...
			gasPrice: this.stats.gasPrice,
			uptime: this.stats.uptime,
			nextBlockTime: (1000 * this.stats.deadline) + _.now()
		}) )
		{
			...
			this.stats.nextBlockTime = (1000 * stats.deadline) + _.now();
